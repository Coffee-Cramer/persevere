...
import {  FACTION_SRG_AS_ALLY,  ILLUSION_GROUP, SCOURGEAPP, SCOURGEAPP2 } from "./A_Faction";
import { MECHANICS } from "../../../../../../Enumerators&Arrays/mechanic_immunity_list";

class Appearance{
    private num: number
    private faction: number = FACTION_SRG_AS_ALLY.ID
...
const ILLUSION_0 = std.Spells.create("shrek6", "ILLUSION_0SRG_A")
...
const ILLUSION_1 = std.Spells.create("shrek6", "ILLUSION_1SRG_A")
    .Name.enGB.set("Illusion: Scourge")
    .Icon.setPath("achievement_dungeon_cotstratholme_25man")
    .Description.enGB.set("Selected appearance.")
    .CastTime.set(0)
...
export const ILLSRG_PARENT_ALLY = std.Spells.create("shrek6", "ILLSRG_PARENT_A")
    .Name.enGB.set("Illusion: Scourge (Alliance)")
    .Icon.setPath("achievement_dungeon_cotstratholme_25man")
    ...
    //Visual
    .Effects.addModTriggerSpell("shrek6", "ILLVISUALSRG_A", 0, x => x
        .Name.enGB.set("Illusion: Scourge")
        .Icon.setPath("achievement_dungeon_cotstratholme_25man")
    ...
    .InlineScripts.OnCast((spell) => {
        ...
        function castSpell(rand: number){
            if(Math.round(rand) == 0){
                caster.CastSpell(caster, 80973, true) 
            }
            if(Math.round(rand) == 1){
                caster.CastSpell(caster, 80974, true) 
            }
        }
...
ILLSRG_PARENT_ALLY.SkillLines.add(AbilityCategory.Exec.getSkill())

//console.log("SCOURGE_ALLIANCE:")
