...
import { FACTION_DI_AS_ALLY, ILLUSION_GROUP } from "./A_Faction";
import { MECHANICS } from "../../../../../../Enumerators&Arrays/mechanic_immunity_list";


const DRKIRN_A = std.CreatureTemplates.create("shrek6", "DRKIRNA", 8901)
    .Models.clearAll()
    .Models.addIds(
        8825,
        //4936,
...
const DRKIRN_A2 = std.CreatureTemplates.create("shrek6", "DRKIRN_A2", 8901)
    .Models.clearAll()
    .Models.addIds(
        //8170,
        825
...
class Appearance{
    private num: number
    private faction: number = FACTION_DI_AS_ALLY.ID
    ...
    public pickApr(inNum: number){ //select the appearance from an enumerator of options
        ...
        if(this.num == this.Enumerator.i1){
            this.num = DRKIRN_A.ID 
        }
        else if(this.num == this.Enumerator.i2){
            this.num = DRKIRN_A2.ID 
        }
...
const ILLUSION_0 = std.Spells.create("shrek6", "ILLUSION_0DI_ASALLY")
...
const ILLUSION_1 = std.Spells.create("shrek6", "ILLUSION_1DI_ASALLY")
    .Name.enGB.set("Illusion: Dark Iron")
    .Icon.setPath("Inv_helmet_08")
    .Description.enGB.set("Selected appearance.")
    .CastTime.set(0)
...
export const ILLDI_PARENT = std.Spells.create("shrek6", "ILLDI_PARENT_ASALLY")
    .Name.enGB.set("Illusion: Dark Iron Dwarves (Alliance)")
    .Icon.setPath("Inv_helmet_08")
    ...
    //Visual
    .Effects.addModTriggerSpell("shrek6", "ILLALLYVISUALDI_ASALLY", 0, x => x
        .Name.enGB.set("Illusion: Dark Iron")
    ...
    .InlineScripts.OnCast((spell) => {
        ...
        function castSpell(rand: number){
            if(Math.round(rand) == 0){
                caster.CastSpell(caster, 80951, true) 
            }
            else if(Math.round(rand) == 1){
                caster.CastSpell(caster, 80952, true) 
            }
